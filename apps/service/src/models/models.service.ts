...
export class ModelsService {
  ...
  async update(
    userId: string,
    id: string,
    data: Prisma.ModelConfigUpdateWithoutUserInput,
  ) {
  ...
  async testConnection(config: any) {
    ...
    if (!aiproxyUrl || !aiproxyToken) {
      throw new BadRequestException(
        'AIPROXY_API_ENDPOINT or AIPROXY_API_TOKEN not configured in server environment',
      );
    }
    ...
    try {
      console.log(
        `Testing connection to ${baseURL} with key ending in ...${apiKey.slice(-4)}`,
      );
  ...
  async getMonitorStats(userId: string) {
    ...
    const totalRequests = await this.prisma.modelLog.count({
      where: { userId },
    });
